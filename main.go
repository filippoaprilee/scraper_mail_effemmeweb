...
func main() {
	...
	for {
        ...
        fmt.Println("11. 🔍 Scraping con confronto su file CSV di riferimento.")
		fmt.Print("\n" + color.New(color.FgYellow).Sprint("Scegli un'opzione (1-11): "))
        ...
        case "10":
			...
			return
        case "11":
            fmt.Println("🕵️‍♂️ Avvio scraping con confronto su file di riferimento...")
            if err := runScrapingWithReference(ctx); err != nil {
                fmt.Println(color.New(color.FgRed).Sprintf("Errore: %v", err))
            }                     
		default:
...
func startScrapingWithReference(ctx context.Context, category string, refSet map[string]struct{}) (string, error) {
    ...
    csvWriter.UseCRLF = true // se vuoi terminatori di riga in stile Windows
...
writers := []scrapemate.ResultWriter{
    ...
    if err != nil {
        return "", fmt.Errorf("errore nella configurazione dello scraping per la categoria %s: %v", category, err)
    }
    ...
    // Ottieni le keyword dal file e filtra per la categoria
    ...
    if err != nil {
        return "", fmt.Errorf("errore nella creazione dei lavori di scraping per la categoria %s: %v", category, err)
    }
...
func runScrapingWithReference(ctx context.Context) error {
    // 📂 1. Mostra i file CSV disponibili nella cartella csv_results
    csvDir := filepath.Join("scraper_results", "csv_results")
    files, err := filepath.Glob(filepath.Join(csvDir, "*.csv"))
    if err != nil || len(files) == 0 {
        return fmt.Errorf("nessun file CSV trovato nella directory %s", csvDir)
    ...
    fmt.Println("\n📂 File disponibili come riferimento:")
    for i, file := range files {
        fmt.Printf("%d. %s\n", i+1, filepath.Base(file))
    }

    // 📥 2. Chiedi i numeri dei file da usare
    fmt.Print("\nSeleziona i file CSV da usare come riferimento (es. 1,3,5): ")
    reader := bufio.NewReader(os.Stdin)
    input, _ := reader.ReadString('\n')
    input = strings.TrimSpace(input)
    selectedIndexes := strings.Split(input, ",")

    // 📦 3. Salva i file selezionati
    ...
    for _, idx := range selectedIndexes {
        ...
        referenceFiles = append(referenceFiles, files[index-1])
    }

    if len(referenceFiles) == 0 {
        return fmt.Errorf("nessun file valido selezionato")
    }

    // 🧠 4. Carica le email/telefoni/nome attività dai file di riferimento
    referenceSet := make(map[string]struct{})
    for _, file := range referenceFiles {
        err := loadReferenceCSV(file, referenceSet)
        if err != nil {
            fmt.Printf("❌ Errore nel file %s: %v\n", file, err)
        }
    }

    // 📌 5. Scegli una sola categoria da scrapare
    categories, err := extractCategoriesFromKeywordFile("./keyword.csv")
    if err != nil {
        return fmt.Errorf("errore nell'estrazione delle categorie: %v", err)
    }
    ...
    fmt.Print("\nScegli una categoria da scrapare: ")
    choiceStr, _ := reader.ReadString('\n')
    choiceStr = strings.TrimSpace(choiceStr)
    choice, _ := strconv.Atoi(choiceStr)
    if choice < 1 || choice > len(categories) {
        return fmt.Errorf("categoria selezionata non valida")
    }
    selectedCategory := categories[choice-1]

    // ✅ 6. Mostra conferma prima di partire
    fmt.Printf("\nStai per scrapare la categoria **%s** con confronto su %d file di riferimento.\n", selectedCategory, len(referenceFiles))
    if !confirmAction("Procedere? (y/n)") {
        return fmt.Errorf("operazione annullata")
    }

    // 🚀 7. Avvia scraping con writer personalizzato che usa referenceSet
    _, err = startScrapingWithReference(ctx, selectedCategory, referenceSet)
    return err

}

func loadReferenceCSV(filePath string, refSet map[string]struct{}) error {
    ...
    reader := csv.NewReader(file)
    _, err = reader.Read() // salta intestazione
    if err != nil {
        return err
    }
    ...
    for {
        ...
        // Chiave composta per evitare duplicati
        key := strings.ToLower(record[0] + record[7] + record[3]) // nome + email + telefono
        refSet[key] = struct{}{}
    }
...
func NewCustomCsvWriterWithVCFWithReference(w *csv.Writer, vcfFile *os.File, refSet map[string]struct{}) scrapemate.ResultWriter {
    return &customCsvWriter{
        ...
        refSet:  refSet, // 👈 qui viene passato
    }
}
...
func (cw *customCsvWriter) WriteResult(result scrapemate.Result) error {
    ...
    // 🔁 Verifica contro i file di riferimento se presenti
    if cw.refSet != nil {
        key := strings.ToLower(entry.Title + entry.Email + entry.Phone)
        if _, exists := cw.refSet[key]; exists {
            fmt.Printf("🔁 Trovato nei file di riferimento, salto: %s\n", entry.Title)
            return nil
        }
    }
